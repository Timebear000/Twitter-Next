...
import { modalStatus, postIdStatus } from "../store/StatusStore";
const Post = ({ id, post, postPage }) => {
  const { data: session } = useSession();

  // const isOpen = modalStatus((state) => state.modalState);
  const setIsOpen = modalStatus((state) => state.SetModalState);
  // const postId = postIdStatus((state) => state.postIdState);
  const setPostId = postIdStatus((state) => state.SetPostIdState);
  ...
  return (
    <>
      <div onClick={() => router.push(`/${id}`)} className="p-3 flex cursor-pointer border-b border-gray-700">
        {!postPage && <img src={post?.userImg} alt="프로필이미지" className="h-11 w-11 rounded-full mr-4" />}
        <div className="flex flex-col space-y-2 w-full">
          ...
          {postPage && <p className="text-[#d9d9d9] mt-0.5 text-xl">{post?.text}</p>}
